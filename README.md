...
[![Release: v1.6.3](https://img.shields.io/badge/release-v1.6.3-brightgreen)](https://github.com/markpelayo/macos-menubar-RollingCalendar/releases/latest)
...
It's in daily use, and [v1.6.3](CHANGELOG.md) is the current release. What it deliberately doesn't do is listed under [Known limitations](#known-limitations).
...
```
macos-menubar-RollingCalendar 1.6.3  ·  by markpelayo
```
...
The first row of the menu — `macos-menubar-RollingCalendar 1.6.3 · by markpelayo` — opens the [project page](https://github.com/markpelayo/macos-menubar-RollingCalendar). The version is read from the app bundle, so it always names the build you're running.
...
notes — `./release-notes.sh 1.6.3` prints one version's section for a release body. The same notes appear on the
